...
3. **`results/analysis/`** receives the tables and figures generated by the
   replication scripts.
4. **`results/cache/`** receives local intermediate panels that avoid
   rebuilding computationally expensive samples.
5. **`results/logs/`** receives local execution logs.

The directory tree documents the expected replication workflow. The
restricted inputs and locally generated caches, logs, tables, and figures do
not need to be included in a code-only distribution.
