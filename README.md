...
Establishments in Santa Catarina were first filtered from the national RAIS. Geocoding for this subset was then improved using the Google Geocoding API applied to complete addresses and postal codes. Imprecise matches and establishments without a valid geocode were excluded. Exposure is the minimum distance from the establishment's baseline location to the nearest polygon in the 2008 flood map supplied by Marinho et al. (2012). Treated establishments are located 0-5 km from a mapped flood spot; controls are located 50-80 km away. Treatment status remains fixed after relocation. Once treated and control groups were defined, establishments and their 2007 workers were tracked in the national RAIS.

The establishment panel follows the baseline establishments through CNPJ. The main period is 2003-2012, with extensions through 2016. Closure is recorded in an establishment's last year in RAIS, and relocation indicates a change in census tract between consecutive years. The restricted input is `Natural Disaster Santa Catarina - Dataset.dta`.
